...
describe("appStorage", () => {
  ...
  it("returns default image model when not set", () => {
    expect(appStorage.getSelectedImageModel()).toBe(
  ...
  it("returns null for passwordRequired when not set", () => {
    expect(appStorage.getPasswordRequired()).toBeNull();
  });

  it("stores and retrieves passwordRequired true", () => {
    appStorage.setPasswordRequired(true);
    expect(appStorage.getPasswordRequired()).toBe(true);
  });

  it("stores and retrieves passwordRequired false", () => {
    appStorage.setPasswordRequired(false);
    expect(appStorage.getPasswordRequired()).toBe(false);
  });
});
